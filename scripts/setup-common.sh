...
if [[ "${WRITE_ENV:-0}" == "1" ]]; then
    ...
    if [[ -f ".env" ]]; then
        ...
        [[ -n "$(read_env_var NULLEXIT_SEED)" ]] && EXISTING_SEED=$(read_env_var NULLEXIT_SEED)
    fi

    # README/devref both document NULLEXIT_SEED as "generated by setup.sh" — actually
    # generate it (rather than leaving the .env.example placeholder for the user to
    # fill by hand), since crypto.sh --verify hard-fails toggle.sh's very first run
    # without one, and there's nothing else in the setup flow that would create it.
    if [[ -z "$EXISTING_SEED" ]]; then
        EXISTING_SEED=$(openssl rand -hex 32)
        ok "Generated NULLEXIT_SEED for script-integrity signing."
    fi
...
EOF
    ...
    # Sign the core scripts against the (possibly just-generated) seed now, so the
    # very first toggle.sh run doesn't hard-fail crypto.sh --verify with no seed/no
    # .signatures and no indication that a manual `crypto.sh --sign` was needed.
    if bash scripts/crypto.sh --sign >> output.log 2>&1; then
        ok "Core scripts signed for integrity verification."
    else
        warn "Could not sign core scripts — run 'bash scripts/crypto.sh --sign' manually before toggle.sh."
    fi
fi
