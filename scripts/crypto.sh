...
if [ "$1" == "--sign" ]; then
  ...
  rm -f .signatures
  for file in toggle.sh recover.sh scripts/common.sh scripts/routing-fix.sh scripts/toggle-linux.sh scripts/recover-linux.sh scripts/diagnose-host-leak.sh scripts/watcher.sh; do
    if [ -f "$file" ]; then
