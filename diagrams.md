...
flowchart TD
    ...
    ST5["5. stop_sleep_prevention\n(kill caffeinate, rm PID file)"]
    ST6["6. stop_dns_watcher\n(SIGTERM, rm PID file)"]
    ST7["7. stop_warp_watcher\n(SIGTERM, rm PID file)"]
    ST8["8. stop_host_leak_probe\n(SIGTERM, rm PID file)"]
...
flowchart TD
    ...
    subgraph NUCLEAR["☢️ Default (nuclear — gateway torn down)"]
        ...
        N3["3. Disable SOCKS5 proxy\n(all interfaces)"]
        N4["4. Flush DNS cache\n(dscacheutil -flushcache)"]
        N5["5. Flush stale routes\n(WARP endpoint routes)"]
        N6a["6a. docker compose down -t 30\n(stop all containers)"]
        N6b["6b. Kill caffeinate\n(/tmp/nullexit-caffeinate.pid)"]
        N6c["6c. Kill DNS Watcher\n(/tmp/nullexit-dns-watcher.pid)"]
        N6d["6d. Kill Host Leak Probe\n(/tmp/nullexit-host-leak-probe.pid)"]
