...
**nullexit** includes a powerful Python utility (`sync-rules.py`) that runs inside the ephemeral `rule-compiler` Docker container on every startup. It handles two compilation pipelines simultaneously:
...
6. **Container Data Folder Is Off-Limits From Outside Docker**: Never write to `adguard/work/` from the host, another container, or any side script. That folder is bind-mounted into `adguardhome`, `routing-fix`, and the `rule-compiler`; manual `cp`, `rm`, `>`, or editor edits anywhere else will desync the AdGuard in-memory rule engine, corrupt the BoltDB session store (`data/sessions.db` / `data/stats.db`), or break the on-disk filter cache. **The single source of truth for writes inside `adguard/work/` is `python3 sync-rules.py`** — run it on the host to recompile; do not touch individual files from elsewhere. To safely inspect live state, use `docker compose exec adguardhome ls work/data/filters/` (read-only).
