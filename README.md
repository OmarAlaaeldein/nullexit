...
- **Internal Port Obfuscation:** To defend against physical or internal static-inspection of the live Docker environment, the fixed internal WireGuard listening port was removed from the environment variables. Instead, a custom `entrypoint` script was injected into the Cloudflare container that forces the Linux kernel to dynamically generate a randomized ephemeral port between `1024` and `65535` in memory on every single boot. Note that this only obfuscates the *internal* listening port within the container's network namespace; outbound traffic still uses standard MASQUERADE techniques and targets the fixed Cloudflare endpoint port `2408`.
- **Autonomous Service Architecture:** The gateway runs autonomously using a combination of macOS `launchd` (via `brew services start colima`) and Docker's `restart: unless-stopped` policies. When the host Mac boots, `launchd` starts the Colima VM in the background, which in turn triggers the Docker daemon to automatically revive the fully-randomized routing stack without user intervention.
