...
- **macOS:** You can dynamically compile the provided `Toggle-Gateway.applescript` source code into a fully native, clickable macOS Application by running:
  ...
  ```
  This creates a beautiful, self-contained app in the current directory. You can then right-click it and choose "Make Alias", and drag that alias to your Desktop for quick access. This ensures it intelligently toggles the Docker gateway on or off from the correct folder.
...
- **macOS Memory Overhead (OrbStack):** Because macOS cannot run Linux containers natively, a hypervisor must run a background Linux VM. This project recommends and uses **OrbStack** as the Docker engine, which dynamically allocates memory. Since the actual containers (`warp`, `tailscale`, `routing-fix`) only consume roughly `~75MB` of RAM combined, OrbStack will smoothly idle at under ~100MB of total memory overhead without needing the heavy, static 500MB+ memory block required by older tools like Colima or Docker Desktop.
