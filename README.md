...
- **Internal Port Obfuscation:** To defend against physical or internal static-inspection, the hardcoded WireGuard listening port was removed from the environment variables. Instead, a custom `entrypoint` script was injected into the Cloudflare container that forces the Linux kernel to dynamically generate a randomized ephemeral port between `1024` and `65535` in memory on every single boot.
